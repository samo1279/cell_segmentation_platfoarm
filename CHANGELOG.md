...
### Fixed
- **CH5 FIX \#8** (§5.6, §6.3): Aligned the Single-node PostgreSQL limitation with the added PersistentVolumeClaim. §5.6 now states that data persists across pod restarts via a PVC and the remaining limitation is a single pod with no replication, failover, or backup; authentication and history are temporarily unavailable during a restart but resume from persisted data. §6.3 summary updated to match.
- **CH5 FIX \#7 (§5.5):** Framed the external feedback proportionately. Added explicit $(n=1)$ designation; stated the session is an informal domain-relevance check, not a formal usability study or evaluation result; stated no conclusions about usability or practical value can be drawn from a single observation; cross-referenced the \"No formal usability study\" limitation in Section~5.6.

- **CH5 FIX #6** (§5.5 + ch1 §1.1): Acknowledged that the external reviewer indicated \
  manual analysis is not a pressing pain point (Appendix A.4 Q4); reframed the \
  platform's value proposition around consistency, reproducibility, structured automation, \
  and scale rather than relieving an urgent burden. Softened the Chapter 1 background \
  sentence about manual analysis to qualify it as context-dependent ("in high-throughput \
  or multi-observer settings"), keeping it consistent with the reviewer's feedback and \
  supported by the existing citation (ref2). Appendix verbatim answers unchanged.
- `latex_doc/Latex_main/chapters/ch5_evaluation.tex` — **CH5 FIX #5 (TEXT ONLY)**: Added measurement methodology and hardware context to the performance section. (1) Section 5.3 opening now states figures are indicative observations from manual test runs on images of varying sizes/densities with no fixed image set and no statistical analysis. (2) Table 5.2 caption (`tab:latency`) now names the production GPU as NVIDIA A40 (GitLab runner tag `a40gpu`) and notes the observations are not from a controlled benchmark. (3) Table 5.3 caption (`tab:cicd_perf`) replaced "Median Duration" column header with "Observed Duration" and caption now states values are typical observations over several CI runs with no formal sample or statistical analysis. (4) Section 5.7 (Future Work) extended with a sentence noting that a controlled performance benchmark with a fixed image set, repeated runs, and statistical summary is future work.
- `latex_doc/Latex_main/chapters/ch5_evaluation.tex` — **CH5 FIX #4 (TEXT ONLY)**: Resolved contradiction between Sections 5.1 and 5.3 on concurrent-request behaviour. Section 5.3 previously claimed "the semaphore correctly serialized concurrent requests: a second submission queued transparently" — implying measured verification — while Section 5.1 indicated concurrent behaviour was outside the automated suite's scope. Section 5.3 now states that the asyncio.Semaphore(1) gate is **designed** to serialize inference so that concurrent submissions queue rather than compete for GPU memory, and explicitly notes this concurrent behaviour was not part of the automated test suite and was not measured. Section 5.1 updated to explicitly state that concurrent-request behaviour under the gate was not exercised by the suite and was not measured. No unverified verification claims remain in either section.
- `latex_doc/Latex_main/chapters/ch5_evaluation.tex` — **CH5 FIX #3 (TEXT ONLY)**: Sections 5.1 and 5.2 revised to state plainly that segmentation correctness was not evaluated. Section 5.1 now explicitly states that unit tests use a stubbed model (not real Cellpose), so they test API plumbing only; ground-truth comparison was not performed; no accuracy metrics (IoU, AP, Dice) were computed. Section 5.2 now explicitly states that segmentation output was assessed by visual inspection only; no ground-truth or quantitative evaluation was performed; results confirm output production, not correctness. Both sections emphasize consistency with proof-of-concept scope and reference the benchmarking future-work item in Section 5.7.
...
- `latex_doc/Latex_main/chapters/ch5_evaluation.tex` — Section~5.2 (Segmentation Results) opening paragraph expanded to state explicitly: (1) figures were produced by real Cellpose inference, unlike unit tests which use a stubbed model; (2) assessment was by visual inspection of representative images only; (3) no ground-truth comparison and no quantitative accuracy metrics (IoU, AP, Dice) were computed; (4) this is consistent with the proof-of-concept scope and the benchmarking future-work item in Section~5.7.
